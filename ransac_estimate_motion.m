...
function [best_model, fail]=ransac_estimate_motion(S1, S2, ransac_iterations, num_random_samples, max_allowable_sample_error, min_points_in_consensus_multiplier),  
  ...
  for i=1:ransac_iterations,
      ...
      maybe_model=least_squares_cartesian(S1(:,maybe_inliers), S2(:, maybe_inliers));
      ...
      % this_model := model parameters fitted to all points in consensus_set          
        this_model=least_squares_cartesian(S1(:,consensus_set), S2(:,consensus_set));
